...
class LitigationCasesViewSet(CSVRendererMixin2, viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        obj = LegalCache(LitigationCases, 1).get_object()
        return obj
    ...
    def get_queryset_old(self):
        ...
        queryset = LegalCache(LitigationCases).get_objects()
    ...
    def get_queryset(self):
        ...
        if internal_ref_number:
            filters &= Q(internal_ref_number=internal_ref_number)

        if characteristic:
            filters &= Q(characteristic=characteristic)

        if start_time:
            try:
                req_date = datetime.strptime(start_time, "%Y-%m-%d").date()
                filters &= Q(start_time__year=req_date.year, start_time__month=req_date.month, start_time__day=req_date.day)
            except ValueError:
                print("Invalid date format for start_time")

        if stage:
            filters &= Q(stage__id=stage)
        ...
        if is_superuser or is_manager or (is_cases_private_manager and is_cases_public_manager):
            user_filter = Q()
        elif is_cases_public_manager:
            user_filter |= Q(case_category="Public")
        elif is_cases_private_manager:
            user_filter |= Q(case_category="Private")
